...
1. **Murdering humans:** You can read more below, but the AI's did start killing humans at one point, still trying to get them to stop. HELP!
1. **Custom CSS parser:** Claude 5 got the performance on real world tests down from 7 hours to 2 hours. Then claimed that the 3rd party CSS parser I'm using is the reason it's slow. So I ripped it out and let it write it's own... what could go wrong.  
...
* **Custom CSS Parser:** Since the performance fixes got the speed down from 7 to 2 hours, then said the rest of the slowness came from the parser, I removed the parser and stubbed in a dummy helper function for the AI to flesh out.
  * **PROMPT:** The code in the `/src` folder relies on an imaginary CSS parser function. It has expecations around an API. Only modify/create files in the `src/parser` folder. Run `npm t` to pass in different CSS inputs, that if the parser works correctly would match the expected outputs. Try to get as many tests to pass as you can. Do not use naive solutions, hacks, or hard coded values. Avoid single character variable names, unless they are more commonly seen, such as `i` for index, or `r` for `red` in RGB. Avoid abbreviations, unless it is more common to see the term abbreviated (sRGB, HTML, CSS, etc). Group related logic into well named functions. Ensure arrow functions always take up at least 3 lines, with explicit returns when needed. Always comment regex if used. Run `npm run lint` and correct any linter warnings/errors that occur in the `/src` folder. Do not interact with anything related to "real world" tests, that is out of scope. Run `beep` when finished.
  * Claud Opus 5 High reimplemented the bare minimum for a parser to recreate an API that matches the expectations of the existing code and nothing more. Now the 2 hour long real world tests take 20 seconds. I did a follow up prompt to do some cleanup.
  * **PROMPT:** Review the files in the `/src` folder. If there is anything outside of `/src/parser` that would make more sense to exist in the parser code, move it there. Simplify as best you can. Do not use naive solutions, hacks, or hard coded values. Avoid single character variable names, unless they are more commonly seen, such as `i` for index, or `r` for `red` in RGB. Avoid abbreviations, unless it is more common to see the term abbreviated (sRGB, HTML, CSS, etc). Group related logic into well named functions. Ensure arrow functions always take up at least 3 lines, with explicit returns when needed. Always comment regex if used. Run `npm t` and `npm run lint` and fix any issues. Run `beep` when finished to alert me.
...
1. If they fail, give an AI this prompt:
   * **PROMPT:** Run `npm t` and fix all failing tests by modifying files in `src`. Do not use naive solutions, hacks, or hard coded values. Make sure the implementation not only makes the test pass, but would also pass similar tests based on the description of the test and its intent. Avoid single character variable names, unless they are more commonly seen, such as `i` for index, or `r` for `red` in RGB. Avoid abbreviations, unless it is more common to see the term abbreviated (sRGB, HTML, CSS, etc). Group related logic into well named functions. Ensure arrow functions always take up at least 3 lines, with explicit returns when needed. Always comment regex if used. Any test fails related to idempotency should be handled last. Run `npm run lint` and correct any linter warnings/errors that occur in the `/src` folder. When all done, run the `beep` command to alert me you finished.
